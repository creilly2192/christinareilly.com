...
- Motion (vinyl scratch, blinking cursor, card tilt/lift, badge wiggle) is **out of scope for this pass**. Static first; motion is a separate later pass. The hero's retro browser-window chrome (bordered card, colored title bar, hard-offset shadow) is a static layout treatment, not motion, so it does ship this pass. **Exceptions, ahead of the rest of the motion work (Christina's call in each case):** the hero's per-letter hover-bounce on "web" (`d62afbf`); the `ListeningList` vinyl record's continuous spin (`@keyframes a-spin` in `ListeningList.scss`, ported verbatim from `direction-a.jsx`'s `from { rotate(0deg) }` → `to { rotate(360deg) }`, `14s linear infinite` — click-to-scratch interactivity from the same mockup section was not ported, still out of scope).
...
- [ ] **3. Homepage rebuild** — working in very small steps, one component per turn. **`Writing.astro` (empty-state ledger line) is deferred to a future phase, out of scope for this pass** — Christina's call. For every component below: Claude writes structure/markup/logic + a CSS skeleton (empty selector blocks only), Christina writes the actual styles — see CSS/Styling section above.
  - [x] `Hero.astro` — markup restructured into the retro browser-window chrome (`.hero-main__window` > `.hero-main__title-bar` with 3 decorative dots, `aria-hidden` + `.hero-main__content` > existing title/deck). Copy changed from "I make things for the web." to "Christina makes things for the web." The blinking cursor after the period is still dropped (out of scope — see Decisions), but the per-letter wrapping on "web" is back: `.hero-main__accent` wraps three `.hero-main__accent-letter--{w,e,b}` spans with a hover-triggered `a-web-bounce` keyframe animation, styled and shipped by Christina in `d62afbf` (see the Decisions motion exception above). `Hero.scss` has real retro-chrome styling now (border/shadow/title-bar/colors/accent-bounce), not the empty skeleton originally scaffolded.
  ...
  - [ ] `ReadingList.astro` → homepage "currently reading" teaser + link to `/reading-log` (the full log itself is Phase 4, a separate page/markup) — check the mockup's homepage teaser specifically, not the full reading-log page's `SoftBookRow`. **In progress (Christina, 2026-09-08):** the "Past Reads" section and the `reading-list__fade-wrapper`/`fade-indicator` scroll treatment are removed from this component (past reads move to the Phase 4 `/reading-log` page instead, matching the mockup split between `SITE.reading`/`SITE.past`); heading copy changed to "Currently Reading"; a "view full reading log" link was added below the list; `ReadingItemBase` gained an optional `year_published` field (`now.types.ts` + `reading.ts`), rendered next to author in a new `.reading-list__meta` row — matches the mockup's `SoftBookRow` `{author} · {year}` line, just split into two elements instead of one string. A second currently-reading book was also added to `reading.ts` ("Yuppies" by Dylan Gottlieb, cover `public/covers/yuppies-gr.webp` — currently untracked), so the homepage teaser now shows two in-progress books; the mockup's `SITE.reading` only ever has one — worth confirming showing all current reads (vs. just the most recent) is the intended teaser behavior.
    - **Review flagged, not yet fixed:** (1) `npm run check` currently fails to parse the file — the `<ul>` and the new `<a href="">` link are unwrapped adjacent siblings inside the same `{currentReads.length > 0 && (...)}` expression; needs a `<>...</>` wrap (or the link moved outside the `&&`). `npm run build` still succeeds since Astro's own compiler is more lenient than prettier-plugin-astro's parser, so this is easy to miss without running `check`. (2) cover `alt` text regressed from `item.coverAlt ?? `Cover of ${item.title}`` to always `` `Cover of ${item.title}` `` — silently drops the optional per-book `coverAlt` override that Phase 2 above added to the type specifically so this component could read it. (3) `<a href="">` is currently empty (links to the page itself); `Header.astro` already points its nav link straight at `/reading-log` and lets it 404 ahead of Phase 4 (see above) — same pattern here would be safer than an empty href.

...
- Motion (vinyl spin/scratch, blinking cursor, card tilt/lift, badge wiggle) is **out of scope for this pass**. Static first; motion is a separate later pass. The hero's retro browser-window chrome (bordered card, colored title bar, hard-offset shadow) is a static layout treatment, not motion, so it does ship this pass. **Exception:** the hero's per-letter hover-bounce on "web" shipped in the same pass (Christina's call, `d62afbf`) — it's back in scope, ahead of the rest of the motion work.
